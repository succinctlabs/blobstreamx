...
pub fn get_validators_as_input<const VALIDATOR_SET_SIZE_MAX: usize, F: RichField>(
    ...
    block_validators
        ...
        .filter_map(|validator| validator_set.validator(validator.address))
        .enumerate()
...
pub fn get_validators_fields_as_input<const VALIDATOR_SET_SIZE_MAX: usize, F: RichField>(
    ...
    let num_sigs = trusted_block.commit.signatures.len();

    validator_set
        .validators()
        .iter()
        .take(num_sigs)
        .filter_map(|v| validator_set.validator(v.address))
        .for_each(|validator| {
            let val_bytes = validator.hash_bytes();
